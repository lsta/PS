...
$wmistuff = get-wmiobject win32_computersystem -ComputerName localhost | select DNSHostName, Manufacturer, Model, SystemType , @{Name="TotalPhysicalMemoryInMB";Expression={"{0:n2}" -f($_.TotalPhysicalMemory/1mb)}}, NumberOfLogicalProcessors, NumberOfProcessors, CurrentTimeZone, DaylightInEffect
$cimstuff = Get-CimInstance Win32_OperatingSystem | Select-Object  Caption, ServicePackMajorVersion, OSArchitecture, BootDevice,  BuildNumber, CSName      

$final = Join-Object -Left $wmistuff -Right $cimstuff -LeftJoinProperty DNSHostName -RightJoinProperty CSNAME -Type AllInLeft -RightProperties Caption
